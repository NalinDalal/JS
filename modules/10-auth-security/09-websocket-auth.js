...
server.listen(0, async () => {
  ...
  console.log("\nAuth options recap:");
  console.log("  1) one-time token in query string (works everywhere, but lands in logs)");
  console.log("  2) HttpOnly cookie in handshake (browser default, no token in URL)");
  console.log("  3) subprotocol/Authorization header (server-to-server clients)");
  console.log("Mid-session expiry: server closes with 4001/4401; client reconnects");
  console.log("with a fresh token. Ping/pong keeps dead connections detected.");
  process.exit(0);
});