...
console.log("defenses    : escape output, sanitize, CSP script-src, HttpOnly cookies, no innerHTML with user data");

console.log("\n--- CSRF (Cross-Site Request Forgery) ---");
// Attacker's page FORCES the victim's browser to send an authenticated request to your site
console.log("attack      : <form action='https://bank.com/transfer' method=POST>...<img src=... onload='form.submit()'>");
console.log("why it works: cookies auto-attach to the request — the server thinks the USER sent it");
...
console.log("defense 2   : CSRF token — server-issued secret in a hidden field/header, checked per request");
console.log("defense 3   : verify Origin/Referer header");
console.log("best        : SameSite=Strict cookies + CSRF tokens; JWTs in memory avoid the whole class");

console.log("\n--- CORS (cross-origin resource sharing) ---");
console.log("PITFALL     : CORS is a BROWSER enforcement tool, not a server security wall.");
console.log("            : curl/scripts/WS are not blocked; it only stops YOUR page from READING the response.");
...
console.log("never       : Access-Control-Allow-Origin: *  with credentials (browser rejects anyway)");
...
console.log("\n--- Extra hygiene ---");
console.log("rate limiting / lockout -> slow credential stuffing & brute force");
console.log("never roll your own crypto; keep secrets out of the client entirely");
console.log("JWT gotchas: sig-stripping (pin algorithm), disclosure of alg:none / HS256-leak from public key");