...
(async () => {
  ...
  console.log("\ntimingSafeEqual: constant-time regardless of how many bytes match");
  console.log("naive ===: returns at first mismatch — enough requests leak the hash byte-by-byte");

  console.log("\nRules: never store plaintext, never write your own hash, use bcrypt/argon2/scrypt,");
  console.log("store salt+params+hash together, verify with a timing-safe compare.");
})();