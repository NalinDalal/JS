...
- **Live bindings**: ESM imports are references, not snapshots — if the source module reassigns an exported `let`, every importer sees the new value on the next read.
- Scope: default/namespace/named imports are **all read-only** in the importing module — assigning `PI = 5` throws `TypeError`, and `export default` mutations only matter when you export a mutable object.
- `import * as ns` doesn't include the default export; `ns.default` is where it lands.
- CORS + `file://`: `<script type="module">` won't load from `file://` (module fetch is CORS-gated) — serve over HTTP or it fails silently in the console.
- Modules are **deferred and singleton** — the module body runs once, so side effects run once; importing the same module 40 times doesn't re-run it (important for initialization order and `Symbol.for` sharing).
...
- **Iterators are one-shot**: once `done: true`, calling `next()` again keeps returning `{ done: true }` — you can't "reset" an iterator (get a fresh one from `[Symbol.iterator]()` again).
- `for...of` on an **infinite** iterator never ends on its own — that's exactly why the Fibonacci example needs `break`; spread (`[...fibonacci]`) on an infinite iterable **hangs forever**.
- Strings are iterable by **code point**, not code unit: `[...str]` handles emoji correctly, `.length`/`charAt` don't (see module 01).
- Plain objects are **not iterable** — `for...of {}` throws `TypeError`; use `Object.entries()` or `for...in`.
- `arguments` is iterable but array-likes like `NodeList`/`HTMLCollection` depend on the runtime; when in doubt, `Array.from()` first.
...
- `return` inside a generator yields `{ value, done: true }` **once** — code after it never runs, and further `next()` calls return `{ value: undefined, done: true }`.
- Calling `gen.return(x)` from outside **terminates** the generator (running cleanup in `finally`, if any); `gen.throw(err)` resumes at the `yield` with the error thrown there.
- Generators are **lazy and stateful**: each call to `countTo(3)` creates an *independent* iterator — a shared generator object can't be reused for two loops.
- Two-way communication is subtle: the *first* `next()` only runs to the first `yield`; arguments passed to the first `next()` are ignored (there's no code to receive them yet).
- `yield*` flattens one level per delegation and propagates `return` values from the inner generator to the outer via its own return — useful for composition, easy to over-nest.
...
- **Symbols are not private**: `Object.getOwnPropertySymbols(user)` finds the "private" key — it's collision-proof, not secrecy. For true privacy use `#` class fields.
- `JSON.stringify` **skips symbol-keyed properties** entirely (they vanish from the output).
- `Object.keys`/`for...in` skip them; only `Object.getOwnPropertySymbols` and `Reflect.ownKeys` see them — a common "where did my property go" bug when mixing symbol keys with spread (`{...user}` drops them too).
- Symbols aren't auto-stringified: `Symbol("x") + ""` throws `TypeError` (unlike strings); use `String(sym)` or `.description`.
- `Symbol.for` is **global across realms/reloads** (same key → same symbol in every iframe) — great for protocol sharing, dangerous if two libraries use the same key by accident.
- Auto-boxing gotcha: `Symbol.toPrimitive` in the class in this section works, but the myriad built-ins (date, array) each do their own coercion — don't expect one hint path everywhere.
...
- **Traps must return correct values or the proxy throws** — e.g. a `set` trap returning `false` (or nothing) makes strict-mode writes throw `TypeError`, and `get` traps must return values or wrap with `Reflect.get` for correctness (`receiver` matters for getters).
- `this` inside methods: calling `obj.method()` through a proxy runs the method with `this = proxy` — code that relies on `this` identity (e.g. `class X extends Proxy` internal slots, private fields) breaks; built-ins like `Map`/`Date` often throw "invalid receiver" on proxied method calls.
- **Proxies are invisible to the wrapped code but not to comparisons**: `proxy === target` is `false`; `Object.is`, `instanceof` and identity checks leak the fact that it's a proxy.
- `Reflect` methods have the *same signatures* as traps and return booleans (e.g. `Reflect.set` → `true`/`false`) — always forward via Reflect, never `target[prop] = v` (which bypasses invariant checks).
- Don't proxy objects you don't control (framework internals, frozen objects — `preventExtensions` traps throw on non-extensible targets), and beware performance: every access through the proxy has trap overhead.
...
- Cleanup also runs on **early exit** — a `return`/`throw` inside the block still disposes (that's the whole point over manual `close()` calls which get skipped on exceptions).
- **Scope is block-level**: `using conn = ...` binds only inside the enclosing `{}` — declaring at function top disposes at function exit.
- Dispose runs in **reverse declaration order** (LIFO) — later resources clean up before earlier ones, mirroring try/finally nesting.
- If both the body and `[Symbol.dispose]` throw, the dispose error **wins** (the original error is suppressed/wrapped).
- `using` requires the value to actually implement `Symbol.dispose` — passing a non-disposable throws `TypeError`, and `await using` (async) needs `Symbol.asyncDispose`; check Node/browser support before adopting it in production.
