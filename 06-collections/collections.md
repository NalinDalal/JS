...
- `new Array(5)` creates **holes**, not `undefined`s — `[1, , 3]` vs `[1, undefined, 3]` differ in `.map`/`.forEach` (holes are *skipped*), `1 in arr` (false for holes), and `Object.keys` output. `Array.of(5)` is the literal-behavior alternative.
- `.sort()` without a comparator sorts **lexicographically as strings** — `[1, 10, 2].sort()` → `[1, 10, 2]`. Always pass `(a, b) => a - b` for numbers (this module's Prove It does).
- `sort`, `reverse`, `splice`, `fill`, `copyWithin` **mutate in place** and return the same reference — the caller's array changes; there is no copy made.
- `indexOf` can't find `NaN` (uses strict equality); `includes` can. `find`/`findIndex` take callbacks and are the escape hatch for object lookups (`indexOf` compares by reference).
- `unshift`/`shift` are **O(n)** — pushing to a queue head rescales badly; use a ring buffer or reverse for large data.
- `flat()` defaults to depth **1** — `[4, [5, 6]]` inside the module's example stays nested without `flat(Infinity)`.
- `delete arr[1]` leaves a hole and keeps `length` — use `splice` for removal (module 09 covered this).
...
- **Object keys compare by reference**: `map.get({ id: 1 })` after `map.set(someObj, ...)` returns `undefined` — you must keep the exact same object reference to retrieve.
- `JSON.stringify(map)` produces `{}` (Map has no native JSON support — see 6.6); round-trip with `Array.from(map.entries())` / `new Map(...)`.
- Keys are compared with **SameValueZero**: `NaN` works as a key (`map.set(NaN, 1); map.get(NaN) === 1`), and keys never coerce (`map.get(1)` won't find a `"1"` key — unlike objects).
- `map.forEach((value, key) => ...)` is **value-first**, the reverse of `Object.entries` — a common source of swapped parameters in migrations from objects.
- Re-`set`ting an existing key **doesn't move** it in iteration order; deleting then re-adding moves the entry to the end.
- Iteration order is guaranteed insertion order — but don't read order-sensitive logic from a `Map` you got from user code; `Object.fromEntries(map)` re-keys everything to strings.
...
- **Dedup is by reference, not equality**: two distinct objects with the same `{ id: 1 }` shape are *both* kept — `[...new Set(users)]` only dedups the exact same object appearing twice (the module's Prove It shows 2, not 1). For value-based dedup, stringify (`JSON.stringify`) or use a `Map` keyed by the stable field.
- SameValueZero (covered in Explain It): `NaN` dedups to one entry, `-0` and `0` collapse.
- Sets are **not indexed** — `set[0]` is `undefined`; iterate with `for...of`, `[...set]`, or `set.values()`.
- `add` returns the Set (chaining works), `delete` returns a **boolean** whether an element was removed — useful as an atomic "consume once" check.
- Iteration order is insertion order — a Set used as a FIFO queue must be combined with `keys().next()` or rethink; `shift` doesn't exist.
- `new Set([1, 2])` is fine, but `new Set(5)` throws — Set accepts **iterables**, not numbers.
...
- **Keys must be objects** — `weakMap.set("string", 1)` throws `TypeError: Invalid value used as weak map key`. Primitives are always reachable so weak references don't apply to them.
- No `.size`, no iteration, no `forEach`, no `clear()` — the whole API is `get/set/has/delete`. You cannot introspect contents or convert to an array.
- **Weakness is one-way**: the *key* is weakly held, the *value* is held strongly. A classic memory leak: a WeakMap whose value references its own key (e.g. `{ obj: key }`) — the key is reachable through the value, so nothing is ever collected.
- GC timing is **unpredictable** — you can't observe "when" the entry disappears, and `FinalizationRegistry` callbacks have no ordering guarantees. Never write logic that depends on collection having happened.
- No iteration means **no `WeakMap` → `Map` elevator**: to migrate a WeakMap's contents you must keep the keys yourself. Also, an intended reason — module text says so — but worth repeating: contents are "unreliably iterable by design".
- Key lookup of a *different but structurally identical* object fails (reference identity), same as Map.
...
- **Fixed length**: no `push`, `pop`, `splice`, `shift`, `unshift` — size is set at construction; to grow, copy into a new larger buffer (`set()` onto a fresh ArrayBuffer).
- **Out-of-range values wrap silently** — `new Uint8Array(1)[0] = 300` stores `44` with *no error*. Ints are stored mod 2^8 (or clamped for `Uint8ClampedArray`, which clamps to 255 instead).
- **All views on one buffer alias the same memory** — the module's example writes `intView[0] = 100` then reads `bytes[0] === 100`. A write through any view is visible through all others; off-by-one in `byteOffset`/`byteLength` corrupts sibling views with no warning.
- `length` counts **elements**; `byteLength` counts **bytes** — for `Int32Array`, `length` is 1/4 of `byteLength`. `buffer.byteLength` is in bytes (12 for 3 int32s).
- **Endianness**: DataView `getUint16`/`setUint16` default **big-endian**; TypedArray views use platform order (little-endian on virtually all modern hardware). Mixed usage on the same buffer yields different numbers — pick one convention for a wire format.
- `new Uint8Array("123")` → empty-ish (string is not a number/iterable of numbers) — always pass an array, iterable, or `ArrayBuffer` explicitly. `TextEncoder` covers UTF-8 only.
...
- **Silent data loss**: functions, `undefined`, and Symbols are dropped without error; `NaN`/`Infinity` become `null`; `Map`/`Set`/`RegExp` become `{}`; `Date` becomes a string. Round-tripping `JSON.parse(JSON.stringify(obj))` is *not* a true deep copy for these types — use `structuredClone` when available (or a library) for functions/cloneables.
- **Circular references throw** — `JSON.stringify` throws `TypeError` on cycles; wrap in try/catch or sanitize with a replacer before shipping to a network boundary.
- `JSON.parse` **throws on invalid input** (see module 08 for handling) — untrusted text should always be parsed inside try/catch; there's no "ignore bad" mode.
- Dates don't survive: stringify → `"2024-01-15T00:00:00.000Z"`, parse → a plain **string** (the module's Prove It revives it manually with a reviver — without it you silently compare strings).
- Object keys become **strings** on both directions — `JSON.parse('{"1": 2}')` has key `"1"`, not `1`; duplicate numeric-looking keys collapse.
- `BigInt` values throw `TypeError` on stringify — a gotcha for counters/ids.
- `structuredClone` also has limits: it rejects **functions** and **class instances with methods** (only cloneable built-ins), so "structuredClone is unlimited" is wrong — but it does handle Map/Set/Date/typed arrays.
